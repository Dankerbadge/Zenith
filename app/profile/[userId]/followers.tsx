...
export default function FollowersListScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const followers = await socialApi.getFollowers(profileUserId);
    const normalized = (Array.isArray(followers) ? followers : []).map((row: any) => ({
      id: String(row?.profiles?.id || row?.follower_id || ''),
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <GlassCard>
          {!rows.length ? <Text style={styles.empty}>No followers yet.</Text> : null}
          {rows.map((row) => (
