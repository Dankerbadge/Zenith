-- Make recompute_day resilient to workouts schema drift (day vs day_key).
...
declare
  ...
  v_steps integer := 0;
  v_input_hash text;
  v_workouts_has_day_key boolean := false;
begin
  ...
  select exists (
    ...
    where c.table_schema = 'public'
      ...
      and c.column_name = 'day_key'
  )
  into v_workouts_has_day_key;

  if v_workouts_has_day_key then
    select
